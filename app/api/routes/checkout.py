...
async def iniciar_checkout():
    """
    🚫 DESATIVADO: autocadastro publico fechado. A cobranca agora e por uso,
    negociada com o time - conta e criada por um super admin via
    POST /planos/admin/ativar-conta depois do contato (WhatsApp).

    A logica de criacao de tenant continua viva em
    app/services/tenant_provisioning.py::criar_escola_com_admin, so que
    chamada pelo endpoint admin em vez deste.
    """
    raise HTTPException(
        status_code=status.HTTP_410_GONE,
        detail="Cadastro automatico desativado. Fale com a gente pelo WhatsApp para ativar sua conta.",
    )
